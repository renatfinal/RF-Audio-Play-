...
const LyricsRenderer = ({ lyrics, duration, audioRef }: { lyrics: string, duration: number, audioRef: React.RefObject<any | null> }) => {
...
export default function RFAudioPlayer() {
  ...
  const handleLocalFiles = async (e: React.ChangeEvent<any>) => {
  ...
  const handlePlaylistImport = async (e: React.ChangeEvent<any>, playlistId: string) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;
  ...
  const shareTrack = async (track: Track, e: React.MouseEvent) => {
    ...
    const downloadFallback = () => {
        if (track.fileBlob) {
            ...
            showToast("O arquivo foi baixado para você enviá-lo.");
        } else if (track.url) {
